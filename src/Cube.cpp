...
Cube::Cube(std::uint32_t attrib_binding, std::span<const AttribConfig> attrib_configs) {
    ...
    for (const auto attrib_config : attrib_configs) {
        ...
        glVertexArrayAttribFormat(vao_id_, attrib_config.index, attrib_config.size/4, GL_FLOAT, GL_FALSE, offset);
        glVertexArrayAttribBinding(vao_id_, attrib_config.index, attrib_binding);
        offset += attrib_config.size;
    }
    glVertexArrayBindingDivisor(vao_id_, attrib_binding, 0);
    glVertexArrayVertexBuffer(vao_id_, attrib_binding, vbo_id_, 0, sizeof(Vertex));
}
