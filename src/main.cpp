...
int main() {
    ...
    Window win{"cube demo", 640, 480, 
    [](int, const char* error_msg){
        spdlog::error("[glfw3] {}", error_msg);
    },
    [](std::uint32_t, std::uint32_t, std::uint32_t id, std::uint32_t, int, const char *message, const void *) {
        spdlog::error("[glad] {}, id = {}", message, id);
    },
    };

    const std::array<std::filesystem::path, 2> shbin_paths{{
        "shaders/bin/vert.spv",
        "shaders/bin/frag.spv"    
    }};
    ...
    while(!win.shouldClose()) {
        ...
        // NOLINTBEGIN
        mat4x4 proj_mat;
        mat4x4_perspective(proj_mat, std::numbers::pi_v<float>/2.f, static_cast<float>(w)/static_cast<float>(h), .1F, 100.F);
        ...
        const auto[x_angle, y_angle, z_angle] = cube.rotate(x_rot, y_rot, z_rot); 
